...
public class AmazonIndex {
    ...
    public AmazonIndex(String server, String index) throws JSONException, IOException {
        ...
        JsonReader.saveJson(this.amazonEC2.toString(4), this.defaultConfigPathname);
    ...
    public AmazonIndex() {
        ...
        final File f = new File(this.defaultConfigPathname);
        JSONObject aux = new JSONObject();
        
        try (
             InputStream is = new FileInputStream(this.defaultConfigPathname);) {
            
            final String jsonTxt = IOUtils.toString(is);
            aux = new JSONObject(jsonTxt);
        } catch (final IOException ex) {
            try {
                new AmazonIndex(this.server, this.index);
